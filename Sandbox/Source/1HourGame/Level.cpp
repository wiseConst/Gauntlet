...
void Level::OnUpdate(const float DeltaTime)
{
    ...
    m_PillarHSV.x += 0.1f * DeltaTime;
    if (m_PillarHSV.x > 1.0f) m_PillarHSV.x = 0.0f;
...
void Level::OnRender()
{
    ...
    // Background
    Renderer2D::DrawQuad({playerPos.x, 0.0f, 0.0f /*-0.8f */}, {50.0f, 50.0f}, {0.3f, 0.3f, 0.3f, 1.0f});
    ...
    {
        Renderer2D::DrawQuad(pillar.TopPosition, pillar.TopScale, glm::vec3(0, 0, 180.0f), m_TriangleTexture, color);
        Renderer2D::DrawQuad(pillar.BottomPosition, pillar.BottomScale, glm::vec3(0, 0, 0.0f), m_TriangleTexture, color);
    }
...
bool Level::CollisionTest()
{
    ...
    for (auto& p : m_Pillars)
    {
        ...
        {
            if (PointInTri({vert.x, vert.y}, tri[0], tri[1], tri[2])) return true;
        }
