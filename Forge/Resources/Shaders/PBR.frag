...
vec3 getNormalFromMap()
{
    ...
    vec3 Q1  = dFdx(InFragmentPosition);
    vec3 Q2  = dFdy(InFragmentPosition);
...
void main()
{
    vec3 albedo     = pow(texture(u_Albedo, InTexCoord).rgb, vec3(2.2));
    float metallic  = texture(u_MetallicMap, InTexCoord).r;
    float roughness = texture(u_RoughnessMap, InTexCoord).r;
    float ao        = texture(u_AOMap, InTexCoord).r;
    ...
    const vec3 lightPos = vec3(-2.0f, -4.0f, 3.0f);
    const vec3 lightColor = vec3(1.0);

    // reflectance equation
    vec3 Lo = vec3(0.0);
    // calculate per-light radiance
    vec3 L = normalize(lightPos - InFragmentPosition);
    vec3 H = normalize(V + L);
    float distance = length(lightPos - InFragmentPosition);
